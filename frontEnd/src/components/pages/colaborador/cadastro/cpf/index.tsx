...
export default function CadastroCpf() {
  ...
  const saveCliente = async () => {
    await api
      .post(`/api/colaborador/adicionar`, dataCliente)
      .then(response => {
      ...
      <Grid grow gutter="sm">
        ...
        <Grid.Col md={4} lg={2}>
          <TextInput
            ...
            required
            size="xs"
            value={formatarCPFCNPJ(dataCliente?.cpf || '')}
            w={250}
            onChange={event =>
      ...
      <Grid grow gutter="sm">
        ...
        <Grid.Col md={6} lg={10}>
          <Select
            withAsterisk
            w={220}
            label={t('pages.colaborador.cadastro.cpf.inputPerfil')}
            placeholder={t(
              'pages.colaborador.cadastro.cpf.placeHoldInputSenha'
            )}
